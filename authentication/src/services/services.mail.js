...
if (!BREVO_API_KEY) {
    console.warn(
        "⚠️ [EMAIL SERVICE] BREVO_API_KEY is not configured."
    );
} else {
...
const sendEmail = async (to, subject, text, html) => {
    if (!brevo) {
        console.error(
            "❌ [EMAIL SERVICE] Brevo is not configured. Email not sent."
        );
    ...
    try {
        ...
        // Important: throw the error so your
        // registration/login route knows email failed
        throw error;
    }
};
